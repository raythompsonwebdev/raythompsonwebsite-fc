...
try {
  fetch("./js/dev/data/slider-data.json").then(function (response) {
    ...
    slider.forEach(function (slide) {
      ...
      panel.setAttribute("id", "#".concat(slide.hash)); // slider panel image

      var slidepanel = document.createElement("FIGURE");
      slidepanel.setAttribute("class", "slider-panel"); // Slider Header

      var slideHeader = document.createElement("H2");
      slideHeader.textContent = "".concat(slide.title); // image link
      ...
      slidepanelHeadingThree.textContent = "".concat(slide.header);
