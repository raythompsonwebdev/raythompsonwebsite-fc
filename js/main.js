...
document.addEventListener("DOMContentLoaded", () => {
	...
	const navLinks = document.getElementsByClassName("tablink"); // get body and html elements

	const scrollScreen = document.querySelector(["body", "html"]); // get header element

	const siteHeader = document.querySelector(".site_header"); // get all divs with class tab content.

	const scrollElements = document.querySelectorAll(".tabcontent"); // detect page scroll function

	const elementInView = function elementInView(el) {
		const scrollOffset =
	...
	const displayScrollElement = function displayScrollElement(element) {
		element.classList.add("scrolled");
	}; // hide element on page scroll

	const hideScrollElement = function hideScrollElement(element) {
		element.classList.remove("scrolled");
	}; // add animation to element on page scroll

	const handleScrollAnimation = function handleScrollAnimation() {
	...
	const menuToggle = document.querySelector(".menu-toggle"); // create menu variables

	const slideoutMenu = document.querySelector(".site_navigation");
	const slideoutMenuHeight = slideoutMenu.offsetHeight; // mobile menu toggle button
	...
	Array.from(navLinks).forEach((link) => {
		...
		link.addEventListener("click", (event) => {
			// Store hash
			const { hash } = event.target; // check if has empty
